...
import ClientProfileActions from '@/modules/profile/components/client-sections/ClientProfileActions.vue'
...
<template>
  <header class="safe-top flex items-center justify-between gap-3 px-4 pt-2">
    ...
    <h1 class="text-[1.05rem] font-semibold text-foreground">
      {{ locale.t.profile.clientPageTitle }}
    </h1>
    ...
    <div class="relative shrink-0">
      ...
      <div
        v-if="menuOpen"
        class="absolute right-0 top-[calc(100%+0.35rem)] z-20 min-w-[11rem] overflow-hidden rounded-2xl border border-border bg-card py-1 shadow-lg"
      >
        <button
          type="button"
          class="pressable block w-full px-4 py-2.5 text-left text-sm font-medium text-foreground"
          @click="handleEdit"
        >
          {{ locale.t.profile.clientMenuEdit }}
        </button>
        <button
          type="button"
          class="pressable block w-full px-4 py-2.5 text-left text-sm font-medium text-destructive"
          @click="handleLogout"
        >
          {{ locale.t.profile.clientMenuSignOut }}
        </button>
      </div>
    </div>
  </header>
  ...
  <div class="agent-profile-card overflow-hidden p-3 sm:p-4">
    ...
    <div class="mt-3 grid grid-cols-4 gap-1.5">
      ...
      >
        <span class="agent-profile-stat__icon" :class="statIconClass(item.tone)">
